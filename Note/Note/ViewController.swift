...
//        cell.textLabel?.text = models[indexPath.row].title
        ...
        cell.textLabel?.text = textModels[indexPath.row].title
        cell.detailTextLabel?.text = textModels[indexPath.row].note //проверка редакции строк
...
//работа со свайпами
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let readed = UIContextualAction(style: .normal, title: "Mark as Readed") { [weak self] action, view, completionHandler in
            self?.table.cellForRow(at: indexPath)?.backgroundColor = self?.defaultcolor as? UIColor
            completionHandler(true)
        }
